...
- **Speaker Inversion files (verified S4):** `ref_embed` must end in `.speaker.safetensors`; `load_speaker_inversion_payload` reads the `speaker_embedding` tensor as saved and `speaker_inversion_batch_tensors` adds the batch axis, so the tensor must be `(tokens, dim)` — a `(1, tokens, dim)` file fails with an `expand` error (only the training path squeezes it). `dim` = `model_cfg.speaker_dim` (768 for v4.1-Small).
